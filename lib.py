...
def fetch_schedule(leagues: List[str], page_token: str = ""):
    ...
    headers = {
        ...
        'x-api-key': os.getenv('ESPORTS_TOKEN'),
...
def get_calendar_buffer(name, events):
    cal = Calendar()
...
def get_events_by_leagues_and_teams(leagues, fav_teams):
    response = get_events_from_api(leagues)
    cal_events = []
    for e in response:
        if e["type"] == "match":
            le = LolEvent(
